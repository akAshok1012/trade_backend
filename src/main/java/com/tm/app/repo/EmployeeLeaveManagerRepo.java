...
public interface EmployeeLeaveManagerRepo extends JpaRepository<EmployeeLeaveManager, Long> {
	...
	Page<EmployeeLeaveManager> getEmployeeLeaveSearch(PageRequest pageRequest, String fromDate, String toDate, String search, EmployeeLeaveStatus employeeLeaveStatus);
	...
	Page<EmployeeLeaveManager> getEmployeeLeaveStatusWithSearch(String search, PageRequest of, EmployeeLeaveStatus employeeLeaveStatus);
	...
	Page<EmployeeLeaveManager> getEmployeeLeaves(Employee employee,
			EmployeeLeaveStatus employeeLeaveStatus,PageRequest of);
	...
	@Query("select elm FROM EmployeeLeaveManager elm  where elm.employee=:employee AND DATE(elm.updatedAt) BETWEEN TO_DATE(:fromDate,'YYYY-MM-DD') and TO_DATE(:toDate,'YYYY-MM-DD') AND elm.employeeLeaveStatus =:employeeLeaveStatus ")
	Page<EmployeeLeaveManager> getEmployeeLeavesWithDateFilter(Employee employee, EmployeeLeaveStatus employeeLeaveStatus,
			PageRequest of, String fromDate, String toDate);
